...
void branch_and_bound(int *final_sol)
{
  ...
  frac_bound(topNode, topNode->fixed);
  current_best = topNode->val;
  copy_array(final_sol, topNode->solution_vec);
  insert(*topNode);

  struc_sol *node1 = (struc_sol*)malloc(sizeof(struct solution_structure));
  struc_sol *node0 = (struc_sol*)malloc(sizeof(struct solution_structure));

  do{
    QueueSize--;
    for(i=1;i<=Nitems;i++)
      final_sol[i-1]=final_sol[i];

    node1->fixed = node1->fixed+1;
    node0->fixed = node0->fixed+1;

    frac_bound(node1, node1->fixed);
    frac_bound(node0, node0->fixed);
    node0->solution_vec[node0->fixed] = 0;
    node1->solution_vec[node1->fixed] = 1;
    QUIET=1;
    int infeasible0 = check_evaluate_and_print_sol(node0->solution_vec, &total_value, &total_weight);
    int infeasible1 = check_evaluate_and_print_sol(node1->solution_vec, &total_value, &total_weight);
      if(infeasible1 == -1){
        node1 = (struc_sol*)malloc(sizeof(struct solution_structure));
            }
      else{
        frac_bound(node1, node1->fixed);
        if(node1->val > current_best){
          current_best = node1->val;
            copy_array(node1->solution_vec, final_sol);
          insert(*node1);
            }
        }
      if(infeasible0 == -1){
        node0 = (struc_sol*)malloc(sizeof(struct solution_structure));
      }
      else{
        frac_bound(node0, node0->fixed);
        if(node0->val > current_best){
          current_best = node0->val;
          copy_array(node0->solution_vec, final_sol);
          insert(*node0);
            }
        }
}while(node0->val > current_best || node1->val > current_best|| QueueSize>0);
QUIET=0;
}
